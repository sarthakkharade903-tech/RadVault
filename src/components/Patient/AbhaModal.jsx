...
import {
  ...
  Award, Edit3, Smartphone, KeyRound, Sparkles
} from "lucide-react";
import { QRCodeSVG, QRCodeCanvas } from "qrcode.react";
import { supabase } from "../../services/supabase";
import { requestAbhaOtp, verifyAbhaOtp, formatAbhaNumber } from "../../services/abdmService";
...
export default function AbhaModal({ member, onClose, onLinked, initialMode = "auto" }) {
  ...
  // Modes: "edit" (input form) and "card" (official blue card)
  const [viewMode, setViewMode] = useState(
    initialMode === "edit" || !isAlreadyLinked ? "edit" : "card"
  );
  const [activeTab, setActiveTab] = useState("otp"); // "otp" | "manual"
  
  // OTP Verification State
  const [aadhaarOrMobile, setAadhaarOrMobile] = useState(member?.mobile || "");
  ...
  // ─── ABDM Milestone 1 OTP Handlers ──────────────────────────────
  const handleRequestOtp = async () => {
    if (!aadhaarOrMobile.trim()) {
      setError("Please enter your 12-digit Aadhaar number or 10-digit mobile number.");
      return;
    }
    setError("");
    setLoading(true);
    try {
      const clean = aadhaarOrMobile.replace(/\D/g, '');
      const type = clean.length === 12 ? 'aadhaar' : 'mobile';
      const res = await requestAbhaOtp(clean, type);
      setOtpTxnId(res.txnId);
      setOtpSent(true);
      setOtpInfoMsg(res.message);
    } catch (err) {
      setError(err.message || "Failed to send OTP via ABDM Gateway.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOtp = async () => {
    ...
    setLoading(true);
    try {
      const verifiedProfile = await verifyAbhaOtp(otpTxnId, otpInput, member);
      setGeneratedAbha(verifiedProfile.abhaNumber);
      setGeneratedAddress(verifiedProfile.abhaAddress);
      await persistAbha(verifiedProfile.abhaNumber, verifiedProfile.abhaAddress);
      setViewMode("card");
    } catch (err) {
            ...
            <div className="space-y-4 animate-in fade-in">
              ...
              <div>
                <h3 className="text-base font-black text-[#16324F]">
                  {isAlreadyLinked ? "Update Official ABHA" : "Link Ayushman Bharat Health Account (ABHA)"}
                </h3>
                <p className="text-xs text-slate-500 mt-0.5">
                  Link verified national digital health identity for {member.name}.
                </p>
              </div>

              {/* Sub-Tabs: OTP Verification vs Manual Entry */}
              <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200">
                ...
                >
                  ...
                  <span>Aadhaar / Mobile OTP (ABDM)</span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span>Direct 14-Digit ABHA</span>
                </button>
              </div>

              {/* TAB 1: Official ABDM OTP Flow */}
              {activeTab === "otp" && (
                <div className="space-y-3.5 pt-1">
                  {!otpSent ? (
                    <div className="space-y-3">
                      <div className="bg-white border-2 border-blue-200 focus-within:border-blue-500 rounded-2xl p-4 shadow-sm space-y-1">
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest">
                          Aadhaar Number (12 Digits) or Mobile
                        </label>
                        <input
                          type="text"
                          placeholder="e.g. 9158100164 or 12-digit Aadhaar"
                          value={aadhaarOrMobile}
                          onChange={(e) => setAadhaarOrMobile(e.target.value)}
                          className="w-full text-base font-mono font-black text-[#16324F] focus:outline-none placeholder-slate-300"
                          autoFocus
                        />
                        <p className="text-[10px] text-slate-400 font-medium">
                          An official OTP will be dispatched via the ABDM Gateway.
                        </p>
                      </div>

                      <button
                        type="button"
                        disabled={loading || !aadhaarOrMobile.trim()}
                        onClick={handleRequestOtp}
                        className="w-full py-3.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 disabled:from-slate-200 disabled:to-slate-300 text-white font-black text-xs rounded-2xl shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer uppercase tracking-wider"
                      >
                        {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <span>Request ABDM OTP →</span>}
                      </button>
                    </div>
                  ) : (
                    <div className="space-y-3 animate-in fade-in">
                      ...
                      <div className="bg-white border-2 border-emerald-300 focus-within:border-emerald-500 rounded-2xl p-4 shadow-sm space-y-1">
                        <label className="block text-[10px] font-black text-emerald-800 uppercase tracking-widest">
                        ...
                        <input
                          ...
                          className="w-full text-2xl font-mono font-black text-emerald-900 tracking-[0.25em] focus:outline-none placeholder-slate-300 text-center"
                          autoFocus
                        />
                        <div className="flex justify-between items-center pt-1 text-[10px] text-slate-400 font-bold">
                          <span>Sandbox Test OTP: 123456</span>
                          <button
                            type="button"
                      ...
                      <button
                        ...
                        className="w-full py-3.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 disabled:from-slate-200 disabled:to-slate-300 text-white font-black text-xs rounded-2xl shadow-lg shadow-emerald-600/20 hover:shadow-xl transition-all flex items-center justify-center gap-2 uppercase tracking-widest cursor-pointer"
                      >
                        {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <span>Verify &amp; Link Official ABHA ✓</span>}
                      </button>
                    </div>
                  )}
                </div>
              ...
              {/* TAB 2: Direct 14-Digit Input (Classic / Demo Mode) */}
              {activeTab === "manual" && (
