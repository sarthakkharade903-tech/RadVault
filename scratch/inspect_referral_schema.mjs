...
async function runCheck() {
  console.log("Checking schema of public.referrals...");
  ...
  if (ashaAuth.session) {
    const { data: referrals, error } = await ashaClient.from('referrals').select('*').limit(1);
    if (error) {
      console.log("Error querying referrals:", error.message);
    } else {
      console.log("Sample referral fields:", referrals);
    }
  } else {
